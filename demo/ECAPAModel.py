...
class ECAPAModel(nn.Module):
	...
	def train_network(self, epoch, source_loader, target_loader):
		...
		# for num, (item1, item2) in enumerate(zip(source_loader, cycle(target_loader)), start=1):
		# 	self.zero_grad()
		# 	data, labels = item1
		# 	tgt = item2
		...
		# 	nloss, prec       = self.speaker_loss.forward(speaker_embedding, labels)
		# 	embed_tgt		  = self.speaker_encoder.forward(tgt.cuda(), aug = True)
		# 	ndomain_loss = self.mmd(embed_tgt, speaker_embedding)
		# 	nloss = nloss + ndomain_loss			
		# 	nloss.backward()
		# 	self.optim.step()
		# 	index += len(labels)
		# 	top1 += prec
	...
	def eval_network(self, eval_list, eval_path):
		self.eval()
		# vae = VAE().to(device)
		# vae.load_parameters("/home/ubuntu/user_space/ECAPA-TDNN-main/exps/exp4/model/model_vae_0003.model")
		# vae.eval()
		files = []
		embeddings = {}
		...
		for idx, file in tqdm.tqdm(enumerate(setfiles), total = len(setfiles)):
			...
			# Speaker embeddings
			with torch.no_grad():
				# label_tgt = torch.zeros((1, 2)).to(device)
				# label_tgt[:, 0] = 1
				# data_1, _, _, _ = vae(data_1, label_tgt)
				embedding_1, sg1 = self.speaker_encoder.forward(data_1, aug = False)
				#print('0', embedding_1.shape)
				# label_tgt = torch.zeros((1, 2)).to(device)
				# label_tgt[:, 0] = 1
				# embedding_1, _, _, _ = vae(embedding_1, label_tgt)
				embedding_1 = F.normalize(embedding_1, p=2, dim=1)
				# label_tgt = torch.zeros((1, 2)).to(device)
				# label_tgt[:, 0] = 1
				# data_2 = data_2.unsqueeze(1)
				# data_2, _, _, _ = vae(data_2)
				# data_2 = data_2.squeeze()
				# torchfbank = torch.nn.Sequential(   
				# torchaudio.transforms.MelSpectrogram(n_stft=512 // 2 + 1, sample_rate=16000, \
				# 								 f_min = 20, f_max = 7600),
				# torchaudio.transforms.GriffinLim(n_fft=512, win_length=400, hop_length =160, window_fn=torch.hamming_window)
				# )
				# data_2 = torchfbank(data_2)
				# print(data_2.shape)
				embedding_2, sg2 = self.speaker_encoder.forward(data_2, aug = False)
				#print('2',embedding_2.shape)
				# label_tgt = torch.zeros((5, 2)).to(device)
				# label_tgt[:, 0] = 1
				# embedding_2,  _, _, _ = vae(embedding_2, label_tgt)
